...
import {Course, Keyword, Lesson} from '../types/course';
...
export const setCurrentAccount = (user: any): void => {
    localStorage.setItem(localStorageKeys.ACCOUNT, JSON.stringify(user));
};
export const getCurrentAccount = (): any | null => {
...
export const setKeywords = (keywords: Keyword[]): void => {
    localStorage.setItem(localStorageKeys.KEYWORDS, JSON.stringify(keywords));
};
export const getKeywords = (): Keyword[] => {
    const keywords: string | null = localStorage.getItem(localStorageKeys.KEYWORDS);
    return !isNil(keywords) ? JSON.parse(keywords) : [];
};
